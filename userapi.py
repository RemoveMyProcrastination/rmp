...
import couchdb, json, datetime, operator
app = Flask(__name__)
...
#curl -X GET http://localhost:5000/dgraph/<userid>/
    ...
    for app in doc['Appdata']:
        if app != "Total":
            ...
            apps[app] = doc['Appdata'][app][today]

    apps = map(lambda x: {x[0]:x[1]}, sorted(apps.items(), key=operator.itemgetter(1), reverse=True))
    #print(apps)
    dailylist["apps"] = apps
    dailylist["total"] =  doc['Appdata']['Total'][today]
    dailylist["goals"] =  doc['Goals']['Daily']
    #print (dailylist)
...
#curl -X GET http://localhost:5000/wgraph/<userid>/
    ...
    for day in weeksort:
        if day != 'Tot':
            days.append({day: doc['Appdata']['Total'][day]})
    weeklylist["days"] = days 
    weeklylist["total"] = ({"Total": doc['Appdata']['Total']['Tot']})
    weeklylist["goals"] = ({"Goal": doc['Goals']['Weekly']})
